...
namespace TD_Engine
{
    ...
    public class GameplayObjectWriter : ContentTypeWriter<GameplayObject>
    {
        protected override void Write(ContentWriter output, GameplayObject value)
        {
            ...
            output.Write(value.AnimationTime);
            output.Write(value.Animate);
            output.Write(value.Rotate);
            output.WriteObject(value.NFrames);
            output.WriteObject(value.SFrames);
            output.WriteObject(value.EFrames);
            output.WriteObject(value.WFrames);
        }
    ...
    public class GameplayObject
    {
        ...
        [ContentSerializerIgnore]
        public int ActiveTile { get; protected set; }

        [ContentSerializerIgnore]
        public float AnimationTimer { get; protected set; }
        ...
        public Vector2 Origin
        {
            get
            {
                if (texture != null)
                {
                    ...
                    {
                        return new Vector2(texture.Height / 2, texture.Height / 2);
                    }
                }
                else
                {
        ...
        public virtual void Update(GameTime gameTime)
        {
            if (status == ObjectStatus.Active)
            {
                float speed = Session._singleton == null ? 1.0f : Session._singleton.Speed;
                ...
                if (Animate && !Rotate)
                {
                    AnimationTimer += ((float)gameTime.ElapsedGameTime.TotalMilliseconds / 2) * Session._singleton.Speed;
                    AnimateWithoutRotate();
                }
        ...
        private void AnimateWithoutRotate()
        {
            if (AnimationTimer > AnimationTime)
            {
                ...
                {
                    if (ActiveTile >= EFrames[0] && ActiveTile <= EFrames[1])
                    {
                        if (ActiveTile < EFrames[1])
                        {
                            ActiveTile++;
                        }
                        else
                        {
                            ActiveTile = EFrames[0];
                        }
                    }
                    else
                    {
                ...
                //Go north
                else if (rotation < -0.75f && rotation > -2.25f)
                {
                    if (ActiveTile >= NFrames[0] && ActiveTile <= NFrames[1])
                    {
                        if (ActiveTile < NFrames[1])
                        {
                            ActiveTile++;
                        }
                        else
                        {
                            ActiveTile = NFrames[0];
                        }
                    }
                    else
                    {
                ...
                {
                    if (ActiveTile >= SFrames[0] && ActiveTile <= SFrames[1])
                    {
                        if (ActiveTile < SFrames[1])
                        {
                            ActiveTile++;
                        }
                        else
                        {
                            ActiveTile = SFrames[0];
                        }
                    }
                    else
                    {
                ...
                {
                    if (ActiveTile >= WFrames[0] && ActiveTile <= WFrames[1])
                    {
                        if (ActiveTile < WFrames[1])
                        {
                            ActiveTile++;
                        }
                        else
                        {
                            ActiveTile = WFrames[0];
                        }
                    }
                    else
                    {
                ...
                DrawRectangle = new Rectangle(ActiveTile * Texture.Height, 0, Texture.Height, Texture.Height);
                AnimationTimer = 0;
            }
        }

        private void AnimateWithRotate()
        {
            if (AnimationTimer > AnimationTime)
            {
                if ((ActiveTile + 1) * Texture.Height < Texture.Width)
                {
                    ActiveTile++;
                }
                else
        ...
        private void CalculateBoundingRectangle()
        {
            if (texture != null)
            {
                if (Animate)
                {
                    rectangle = new Rectangle(0, 0, texture.Height, texture.Height);
                }
                else
                {
        ...
        public virtual void Draw(GameTime gameTime, SpriteBatch spriteBatch)
        {
            if (spriteBatch != null)
            {
                if (texture != null)
                {
                    if (Animate && Rotate)
                    {
                        spriteBatch.Draw(texture, position, DrawRectangle, Color, rotation, Origin, 1.0f, SpriteEffects.None, 0.0f);
                    }
                    else if (Animate && !Rotate)
                    {
                        spriteBatch.Draw(texture, position, DrawRectangle, Color, 0.0f, Origin, 1.0f, SpriteEffects.None, 0.0f);
                    }
                    else if (!Animate && !Rotate)
                    {
        ...
        public class GameplayObjectReader : ContentTypeReader<GameplayObject>
        {
            protected override GameplayObject Read(ContentReader input, GameplayObject existingInstance)
            {
                ...
                result.AnimationTime = input.ReadSingle();
                result.Animate = input.ReadBoolean();
                result.Rotate = input.ReadBoolean();
                result.NFrames = input.ReadObject<int[]>();
                result.SFrames = input.ReadObject<int[]>();
                result.EFrames = input.ReadObject<int[]>();
                result.WFrames = input.ReadObject<int[]>();
